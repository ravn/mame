...
static INPUT_PORTS_START( keyboard )
	...
	PORT_BIT(0x0080, IP_ACTIVE_HIGH, IPT_KEYBOARD) /* 55 */ PORT_CODE(KEYCODE_LALT) PORT_CODE(KEYCODE_RALT) PORT_NAME("Alt") // firmware keymap[55]=0xff (no char) but the boot ROM tracks scancode 0x37 as the Alt modifier: CTRL(0x1D)+Alt(0x37) held + Backspace(0x0E) = the CTRL+ALT+SLET selftest-skip/warm-boot combo
