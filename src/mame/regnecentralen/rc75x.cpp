...
void rc75x_state::nvram_init(nvram_device &nvram, void *data, size_t size)
{
	...
	// than the thin 0x21 two-line underline). byte 0 is chosen so the checksum
	// invariant holds: sum(bytes[0..95]) & 0xff == 0xAA -> byte0 == 0xdf.
	// (A wrong checksum makes the PROM self-test abort with "ERROR: 19" - the
	// NVM fault code - before it ever reaches the floppy.)
